...
Now, we'll do a *bootstrap* sampling procedure. Rather than drawing 500
...
Create a histogram of the variable of interest. Compute the mean and the
median. Which seems like it's a better estimate for the average X?
:::
...
XX. Why might we be uncomfortable using that formula in this setting?
:::

Our goal is to build a confidence interval for the *median* of this
population distribution. Edit the code so that we instead compute and
store the *median* of each bootstrap sample.

```{r}
simulation_results_median = tibble(
  median = rep(NA, 500)
)

for(ii in 1:500){
  x = sample(orig_x, size = 25, replace = TRUE)
  simulation_results_median$median[ii] = mean(x)
}
```
...
and compute the mean and variance of this distribution.
:::

::: callout-note
## Question 7

We've seen a "CLT for medians" in previous course material, which
suggests that the median may be approximately normal. Compute a
...
-   [Yen Chi Chen's lecture
    notes](https://faculty.washington.edu/yenchic/19A_stat535/Lec10_bootstrap.pdf)
